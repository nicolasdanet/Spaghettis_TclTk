...
isAlsa=$(dpkg-query -W -f='${Status}' libasound2-dev 2>/dev/null | grep -c "ok installed")
isJack=$(dpkg-query -W -f='${Status}' libjack-jackd2-dev 2>/dev/null | grep -c "ok installed")

[ ${isTk_5} -eq 1 ]  || [ ${isTk_6} -eq 1 ]  || { echo >&2 "${0##*/}: tk8.6 package required";  exit 1; }
[ ${isTcl_5} -eq 1 ] || [ ${isTcl_6} -eq 1 ] || { echo >&2 "${0##*/}: tcl8.6 package required"; exit 1; }
[ ${isAlsa} -eq 1 ]  || { echo >&2 "${0##*/}: libasound2-dev package required"; exit 1; }
[ ${isJack} -eq 1 ]  || { echo >&2 "${0##*/}: libjack-jackd2-dev package required"; exit 1; }
