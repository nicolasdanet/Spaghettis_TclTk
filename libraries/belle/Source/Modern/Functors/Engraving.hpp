...
#ifndef BELLE_MODERN_FUNCTORS_ENGRAVING_HPP
#define BELLE_MODERN_FUNCTORS_ENGRAVING_HPP
...
void Engravers::Flags::engrave (Music&, NodePtr token, const Pointer < Stamp > & stamp) 
{
    ...
    #else
        stamp->add (Pointer < Graphic > (new Graphic (path, color)));
    #endif
    //
    }
    //
...
void Engravers::Tie::engrave (Music& music, NodePtr token, const Pointer < Stamp > & stamp) 
{
    ...
    Affine affine = Affine::inverse (stamp->getContext());
...
// -----------------------------------------------------------------------------------------------------------
// -----------------------------------------------------------------------------------------------------------
#endif // BELLE_MODERN_FUNCTORS_ENGRAVING_HPP
