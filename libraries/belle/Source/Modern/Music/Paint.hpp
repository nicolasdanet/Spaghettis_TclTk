...
#ifndef BELLE_MODERN_MUSIC_PAINT_HPP
#define BELLE_MODERN_MUSIC_PAINT_HPP
...
{
    if (painter.originIsTopLeft()) { 
        Box box = Affine::scale (scale_).appliedTo (bounds_);
        return Points (box.getWidth(), box.getHeight());
        
    } else {
        Pageable* p = dynamic_cast < Pageable* > (&painter);
        PRIM_ASSERT (p != nullptr);
...
void Music::paintStart (Painter& painter)
{
    ...
    if (painter.originIsTopLeft()) { 
        ...
        Pageable* p = dynamic_cast < Pageable* > (&painter);
        PRIM_ASSERT (p != nullptr);
        origin = Affine::translation (Vector (origin_.getX(), p->getPageSize().getY() - origin_.getY()));
    }
    
    /* Margin guard. */
    
    double y = bounds_.getTop() - Affine::inverse (unit).appliedTo (origin_).getY();
...
// -----------------------------------------------------------------------------------------------------------
// -----------------------------------------------------------------------------------------------------------
#endif // BELLE_MODERN_MUSIC_PAINT_HPP
