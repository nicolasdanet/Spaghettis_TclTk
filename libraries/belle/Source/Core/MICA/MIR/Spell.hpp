...
#ifndef MICA_MIR_SPELL_HPP
#define MICA_MIR_SPELL_HPP
...
typedef Tuple < kWindowSize > Window;               /* Cacheable group of concepts. */
...
private:
    ...
    void vectorFindBest (Window & w) const 
    {
        ...
        #else
            std::map < Window, Window >::const_iterator iter = windows_.find (w);
        #endif
...
public:
    int getPenalty (const Interval& interval) const
    {
        ...
        #else
            std::map < Interval, int >::const_iterator iter = intervals_.find (interval);
        #endif
...
private:
    mutable std::map < Interval, int > intervals_;
    mutable std::map < Window, Window > windows_;
