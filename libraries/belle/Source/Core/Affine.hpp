...
public:
    ...
    static Affine inverse (const Affine& affine) 
    {
...
private:
    AffineStack (const AffineStack&);
    AffineStack& operator = (const AffineStack&);
...
public:
    Affine collapse() const
    {
        Affine affine;
        for (int i = 0; i < stack_.size(); ++i) { affine *= stack_[i]; } 
        return affine;
    };
