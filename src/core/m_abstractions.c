...
static int abstractions_fetch (t_abstractions *x, t_symbol *key)
{
    ...
    //
    if (buffer_getSymbolAt (x->x_keys, i) == key) { return i; }
    //
    }
...
t_symbol *abstractions_getName (t_abstractions *x, t_symbol *key)
{
    ...
    if (i >= 0) { return buffer_getSymbolAt (x->x_names, i); }
    ...
    t_symbol **directory)
{
    ...
    if (i >= 0) {
    //
    (*filename)  = buffer_getSymbolAt (x->x_filenames, i);
    (*directory) = buffer_getSymbolAt (x->x_directories, i);
...
static void abstractions_clear (t_abstractions *x)
{
    ...
    for (i = 0; i < buffer_getSize (x->x_keys); i++) {
    //
    pool_relinquish (buffer_getSymbolAt (x->x_keys, i));
    //
    }
