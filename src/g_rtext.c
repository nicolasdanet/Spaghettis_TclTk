...
t_boxtext *rtext_new(t_glist *glist, t_object *who)
{
    ...
    sprintf(x->x_tag, ".x%lx.t%lx", (t_int)canvas_getView(x->x_glist),
        (t_int)x);
    return (x);
}
    ...
    int *indexp)
{
    ...
    int reportedindex = 0;
    t_glist *canvas = canvas_getView(x->x_glist);
...
{
    sys_vGui(".x%lx.c delete %s\n", canvas_getView(x->x_glist), x->x_tag);
}

void rtext_displace(t_boxtext *x, int dx, int dy)
{
    sys_vGui(".x%lx.c move %s %d %d\n", canvas_getView(x->x_glist), 
        x->x_tag, dx, dy);
}
