...
#define OSS_BYTESPERCHAN(width) (AUDIO_DEFAULT_BLOCKSIZE * (width)) 
...
int oss_reset(int fd) {
     ...
     if ((err = ioctl(fd,SNDCTL_DSP_RESET)) < 0)
          post_error ("OSS: Could not reset");
     return err;
}
    ...
    int suggestedblocksize)
{
    ...
    if (oss_blockmode && !skipblocksize)
    {
        ...
        if (ioctl(fd,SNDCTL_DSP_SETFRAGMENT, &param) == -1)
            post_error ("OSS: Could not set or read fragment size\n");
        if (param != orig)
        {
...
static int oss_setchannels(int fd, int wantchannels, char *devname)
{
    ...
    {
        if (0)
            post_error ("OSS: SOUND_DSP_READ_CHANNELS failed %s", devname);
    }
    else
    {
        if (0)
...
whynot:    
    while (param > 1)
    {
        int save = param;
        if (ioctl(fd, SNDCTL_DSP_CHANNELS, &param) == -1)
            post_error ("OSS: SNDCTL_DSP_CHANNELS failed %s",devname);
        else if (param == save) 
            return (param);
        param = save - 1;
    }
        ...
        int blocksize)
{
    ...
    char devname[20];
    int n, i, fd, flags;
    char buf[OSS_MAXSAMPLEWIDTH * AUDIO_DEFAULT_BLOCKSIZE * OSS_MAXCHPERDEV];
    int num_devs = 0;
    int wantmore=0;
    int spread = 0;
    audio_buf_info ainfo;
    ...
    for (n = 0; n < noutdev; n++)
    {
        ...
        if (ioctl(fd, SNDCTL_DSP_GETCAPS, &capabilities) == -1)
            post_error ("OSS: SNDCTL_DSP_GETCAPS failed %s", devname);
    ...
    if (linux_nindevs)
    {
        ...
        read(linux_adcs[0].d_fd, buf,
            linux_adcs[0].d_bytespersamp *
                linux_adcs[0].d_nchannels * AUDIO_DEFAULT_BLOCKSIZE);
        if (0)
            fprintf(stderr, "...done.\n");
    }
...
int oss_send_dacs(void)
{
    ...
    if (!oss_blockmode)
    {
        ...
        output device has fewer than (audio_advanceInSamples) blocks buffered
        already. */
        oss_calcspace();
    ...
    if (idle && !oss_blockmode)
    {
        ...
        {
            audio_buf_info ainfo;
            interface_monitorBlocking(2000);
            oss_calcspace();
            ...
            no output device should have less than audio_advanceInSamples-1
            */
    ...
    for (dev=0, thischan = 0; dev < linux_noutdevs; dev++)
    {
        ...
        else
        {
            ...
            {
                for (i = AUDIO_DEFAULT_BLOCKSIZE,  fp1 = audio_soundOut +   
                    AUDIO_DEFAULT_BLOCKSIZE*thischan,
                    sp = (t_oss_int16 *)buf; i--; fp1++, sp += nchannels)
                {
                    for (j=0, fp2 = fp1; j<nchannels; j++, fp2 += AUDIO_DEFAULT_BLOCKSIZE)
                    {
                        int s = *fp2 * 32767.;
                        if (s > 32767) s = 32767;
    ...
    for (dev = 0, thischan = 0; dev < linux_nindevs; dev++)
    {
        ...
        {
            for (i = AUDIO_DEFAULT_BLOCKSIZE,fp1 = audio_soundIn + thischan*AUDIO_DEFAULT_BLOCKSIZE,
                sp = (t_oss_int16 *)buf; i--; fp1++, sp += nchannels)
            {
                for (j=0;j<nchannels;j++)
                    fp1[j*AUDIO_DEFAULT_BLOCKSIZE] = (float)sp[j]*(float)3.051850e-05;
            }
        }
        thischan += nchannels;    
     }
