...
void canvas_menuarray(t_glist *canvas)
{
    ...
    sprintf(cmdbuf, "::dialog_array::pdtk_array_dialog %%s array%d 100 3 1\n", gcount);
...
void garray_properties(t_garray *x)
{
    ...
    sprintf(cmdbuf, "::dialog_array::pdtk_array_dialog %%s %s %d %d 0\n",
...
void garray_arrayviewlist_new(t_garray *x)
{
    ...
    x->x_listviewing = 1;
    sprintf(cmdbuf,
            "::dialog_array::pdtk_array_listview_new %%s %s %d\n",
            x->x_realname->s_name,
            0);
    gfxstub_new(&x->x_gobj.g_pd, x, cmdbuf);
                                   ...
                                   t_float fTopItem)
{
    ...
    if (page < 0) {
      page = 0;
      sys_vgui("::dialog_array::pdtk_array_listview_setpage %s %d\n",
               x->x_realname->s_name,
               (int)page);
    }
...
{
    x->x_listviewing = 0;
    sys_vgui("::dialog_array::pdtk_array_listview_closeWindow %s\n",
             x->x_realname->s_name);
}
/* } jsarlo */
...
void garray_redraw(t_garray *x)
{
    ...
    {
      if (x->x_listviewing)
        sys_vgui("::dialog_array::pdtk_array_listview_fillpage %s\n",
                 x->x_realname->s_name);
    }
    /* } jsarlo */
}
