...
void audio_open (void)
{
    ...
    sys_get_audio_params (&m, i, j, &n, o, p, &sampleRate, &advance, &callback, &blockSize);
...
void global_audioAPI(void *dummy, t_float f)
{
    ...
    if (newapi)
    {
        if (newapi == audio_api)
        {
            if (!audio_isOpened() && audio_shouldkeepopen())
                audio_open();
        }
        else
        {
