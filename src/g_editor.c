        ...
        x->gl_editor->e_onmotion == MA_RESIZE)
    {
            ...
            !(x->gl_editor->e_selection->sel_next))
        {
                    ...
                    (gl2 = glist_finddirty((t_glist *)g)))
            {
                ...
                x->gl_editor->e_onmotion = MA_NONE;
                sys_vgui(
"com_confirmAction .x%lx {Discard changes to '%s'?} {.x%lx dirty 0;\n} no\n",
...
void glob_verifyquit(void *dummy, t_floatarg f)
{
    ...
    if (f == 0 && sys_perf)
        sys_vgui("com_confirmAction .console {really quit?} {pd quit} yes\n");
    else glob_quit(0);
}
...
void canvas_menuclose(t_canvas *x, t_floatarg fforce)
{
    ...
    else if (force == 0)    
    {
        ...
        {
            sys_vgui("com_confirmAction .x%lx {Close this window?} {.x%lx menuclose 1;\n} yes\n",
