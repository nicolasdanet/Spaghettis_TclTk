...
static int ooura_init( int n)
{
    ...
    if (n > ooura_maxn)
    {
        if (ooura_maxn)
        {
            sys_freeMemory(ooura_bitrev, ooura_bitrevsize);
            sys_freeMemory(ooura_costab, ooura_maxn * sizeof(FFTFLT) / 2);
        }
        ooura_bitrevsize = sizeof(int) * (2 + (1 << (ilog2(n)/2)));
        ooura_bitrev = (int *)sys_getMemory(ooura_bitrevsize);
        ooura_bitrev[0] = 0;
        if (!ooura_bitrev)
        {
        ...
        ooura_costab = (FFTFLT *)sys_getMemory(n * sizeof(FFTFLT)/2);
        if (!ooura_costab)
        {
            post_error ("out of memory allocating FFT buffer");
            sys_freeMemory(ooura_bitrev, ooura_bitrevsize);
