...
static int readsf_tilde_threadOpenLoopRead (t_readsf_tilde * x, int n)
{
...
{
    t_audioproperties p; soundfile_initProperties (&p);
    ...
    pthread_mutex_unlock (&x->sf_mutex);
    //
    } else { error_canNotFind (sym_readsf__tilde__, p.ap_fileName); }
    //
    }
}
