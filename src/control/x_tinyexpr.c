...
void te_freeParameters (te_expr *x)
{
    if (x) {
    //
    int i;
...
static void te_nextTokenVariableOrFunction (te_state *s)
{
    ...
    while (char_isAlphanumericLowercase ((*s->x_p))) { s->x_p++; }
...
void te_nextToken (te_state *s)
{
    ...
    else if (char_isNumeric (c)) { te_nextTokenNumber (s); }
